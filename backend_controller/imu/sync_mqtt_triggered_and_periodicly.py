...
fiware_url = "http://150.140.186.118:1026/v2/entities/IMU_avg/attrs"  
...
bucket = "AutoSenseAnalytics_imu_live"
org = "students"
token = "7S33V8HBn_vNtFkrdsaNje5eLOLNjaz2CepdRvXI5Ed9-nGTYzg8Tn0AYwiKxQ5ioKVe8hNcyoijbkOhYfjirA=="

#mqtt connection details
mqtt_host = '150.140.186.118'
mqtt_topic='autosense/imuavg'
...
def get_last_influx():
    ...
    try:
        ...
        query=f'from(bucket: "{bucket}")\
                |> range(start: -1y)\
                |> filter(fn: (r) => r["_measurement"] == "imu_live")\
                |> filter(fn: (r) => r["_field"] == "accx")\
                |> last()'
        ...
        if tables:
            for table in tables:
                for record in table.records:
                    ...
                    # Convert the UTC time to Athens time
                    athens_tz = pytz.timezone('Europe/Athens')
                    athens_time = utc_time.astimezone(athens_tz)
                    # Format the timestamp in ISO format
                    timestamp = athens_time.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'
...
def write_to_influx_one_point(data):
    ...
    point = Point("imu_live")\
            .field("accx", float(data["accx"]))\
            .field("accy", float(data["accy"]))\
            .field("accz", float(data["accz"]))\
            .field("acc_avg", float(data["acc_avg"]))\
...
def sync():
    ...
    if influx_last and fiware_last:
        ...
        if influx_last_dt >= fiware_last_dt:
            ...
            data=fetch_data('AutoSenseAnalytics_IMU_avg_Demo',['accx','accy','accz','timestamp','acc_avg'],influx_last,fiware_last)
            print(len(data))
            print(data[0])
            ...
            for i in data:
                point=Point("imu_live")\
                        .field("accx", float(i["accx"]))\
                        .field("accy", float(i["accy"]))\
                        .field("accz", float(i["accz"]))\
                        .field("acc_avg", float(i["acc_avg"]))\
